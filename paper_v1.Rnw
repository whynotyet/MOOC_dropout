...
We are interested in the proportions of respondents that reported each reason for dropout across the courses. These proportions are shown in Figure \ref{fig:prop-reason}. The proprtion for each reason was computed as the number of respondents who reported that the reason applied to them divided by the total number of those who responded to the question (reporting that it aplies or that it does not apply). We also computed the median and interquartile range of the proprtions across all courses for each reason. 


%Figure: proportions for each dropout reason for each course
<<prop-reason, echo=FALSE, fig.env='figure*', fig.pos='ht', fig.height=4, fig.width=9, fig.cap='Proportion of learners who indicated each reason for attrition across 17 MOOCs'>>=
...
The feedback survey for each course asked the learners, among other questions, whether or not they have stopped participating before the end of the course, and if they did, what reasons influenced their decision to stop. For each reason, the student was asked to select one of two options: "Did influence my decision to stop" and "Did NOT influence my decision to stop". Compared to a question where the student only checks a box if the reason applies, this design aims to distinguish between respondents who skip reportinga reason and those who express that the reason did not apply to them. We are interested in examining the strength of correlations between these reasons. We computed the correlation matrix for the reasons for each course separately, and then linearly combined these matrices to yield the average correlation matrix shown in Table \ref{tab:study1_rfd_corr}.

\begin{table*}
\label{tab:study1_rfd_corr}
...
In the initial surveys, learners were asked about their reasons for enrollment, and they were asked to mark whether or not each of a set of enrollment reasons \cite{} applies to them. For each course, we computed the correlation matrix of the reasons for dropout with reasons for enrollment. These matrices were then linearly combined to give the matrix shown in Table \ref{tab:study1_rfd_ei_corr}.
...
\caption{Strongest observed correlations ($r >= 0.15) between reasons for disengagement and enrollment intentions}
\center
\small
\begin{tabular}{llr}
\toprule
 Enrollment intention & Dropout reason & $r$ \\
\midrule
