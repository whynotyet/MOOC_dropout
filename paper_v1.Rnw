...
Table \ref{tab:s1sum} provides summary statistics for enrollment, survey response rates, demographics, self-reported dropout, and behavioral indicators for each course. Self-reported dropout was measured in the feedback survey by asking learners if they stopped participating in the course. The three behavioral indicators are the proportions of learners who viewed at least 30, 50, and 80\% of the lecture videos. Information about age, gender, and education level were only available for learners who completed the initial course survey.
...
Learners reported reasons for enrolling in the course (using the OLEI scale \cite{kizilcec2015motivation} on the course survey) and leaving the course (on the feedback survey) on a grid of binary-choice items. The feedback survey for asked learners whether or not they have stopped participating before the end of the course, and if they did, what reasons influenced their decision to stop. For each reason, the student was asked to select one of two options: ``Did influence my decision to stop" and ``Did NOT influence my decision to stop". Compared to a question where the student only checks a box if the reason applies, this design aims to distinguish between respondents who skip reporting reasons and those who express that the reason did not apply to them. We also computed the fraction of videos in the course each learner had watched to relate self-report measures to learner behavior.

We first report the proportion for each reason for dropping out. This was computed as the number of respondents who reported that the reason applied to them divided by the total number of those who responded to the question (reporting that it applies or that it does not apply). We also computed the median and interquartile range of the proprtions across all courses for each reason. The resulting proportions are shown in Figure \ref{fig:prop-reason}. Second, we performed three correlational analyses were performed to investigate associations between these measures.

We examined the strength of correlations between reasons for dropout. The correlation matrix for the reasons were computed separately for each course, and then linearly combined to yield the average correlation matrix shown in Table \ref{tab:study1_rfd_corr}. Following the same procedure, we computed the correlation matrix between reasons to enroll and reasons for attrition. Furthermore, we computed correlations between reported reasons for dropout and certain learner behaviors. Finally, for each learner, we computed the fraction of course videos visited (the learner started playing the video) and fraction watched (the learner stayed with the video for 3 minutes, or for the whole video duration if shorter than 3 minutes). We then computed the matrix of correlations between reasons for dropout and these variables for each course. These matrices were averaged and the result is shown in Table [TableToBeAdded].
...
What reasons do learners report for dropping out of a MOOC? (RQ3a)
