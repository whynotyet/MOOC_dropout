...
<<s1coefs, echo=FALSE, fig.env='figure*', fig.pos='ht', fig.height=6, fig.width=10, fig.cap='Coefficients of predictors (binary, except for normalized age) from logistic mixed-effects model for predicting persistence and achievement milestones (prop. videos watched, prop. assessments completed, and grade percentile). The baseline (intercept) are males in N. America of average age who have not earned a bachelors\', no prior experience in the topic, not intending to complete the course, prepared to spend under 2 hours/week, this being their first MOOC. An estimate of -20 indicates a 20\\% points lower probability of achieving a milestone.'>>=
library(ggplot2)
...
res$y = factor(res$y, labels=c("> 30% videos", "> 50% videos", "> 80% videos", "> 60th percentile grade", "> 30% assessments", "> 50% assessments", "> 80% assessments", "> 80th percentile grade"))
...
summary(lmer(satisfaction~0+prop_vid4 + (1|course_id), sat))
...
The objective was to build an `early warning' prediction model that could flag learners who are likely to have dropped out or to drop out in the future. In order for our predictions to be useful, we need to observe the following three features of the dropout prediction problem.
...
The almost identical results obtained for $test_1$ and $test_2$ imply good transferability of the model.
...
The present studies extend the empirical base of research on attrition in online learning environments. The key findings are i. the presence of strong individual differences in persistence by demographic and geographic features, ii. the contribution of learners' low capacity for volitional control 
