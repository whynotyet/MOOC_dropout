...
First, we compared the distributions of satisfaction scores for learners who reported they stopped participating and those who reported not to have stopped. A significant difference was observed ($t_{1469}=20.7, p \ll .0001, d=0.7$). The mean satisfaction score in the group that reported stopped (no stopped) participation was 4.70 (5.34). 
...
<<study1_satsfaction_by_vid_groups, echo=FALSE, fig.env='figure*', fig.pos='ht', fig.height=4, fig.width=9, fig.cap='Satisfaction index scores for learners who reported they stopped / did not stop participating before end of course'>>=
b = a[!is.na(a$did_dropout),]
course_ids = unique(a$course_id)
course_num_videos = list()
for (cid in course_ids) {
  course_num_videos[[cid]] = quantile(a$num_videos_watched[a$course_id == cid], 0.95, na.rm=TRUE)
}
for(i in 1:nrow(b)) {
  x = b[i,]
  f = x[['num_videos_watched']]/course_num_videos[[x[['course_id']]]]
  if (is.na(f)) {level=NA}
  else if (f <= 0.25) {level = "0-25%"}
  else if  (f <= 0.50) {level = "25-50%"}
  else if  (f <= 0.75) {level = "50-75%"}
  else {level = "75-100%"}
  b[i,"fraction_videos_watched"] = level
}
b$fraction_videos_watched = factor(b$fraction_videos_watched)
c = b[!is.na(b$fraction_videos_watched),]
ggplot(c) + geom_boxplot(aes(x=c$fraction_videos_watched,y=c$satisfaction)) + theme_bw() + labs(y="Satisfaction",x="Fraction videos watched")
@
...
<<s1reason, echo=FALSE, fig.env='figure*', fig.pos='ht', fig.height=4, fig.width=9, fig.cap='Proportion of learners who indicated each reason for attrition across 17 MOOCs'>>=
