...
Every learner who was predicted to disengage from the course was sent an email kindly requesting their help: ``You are enrolled in [course name], but you've been less active recently. Could you help us understand why?'' A very low response rate was expected, given that this subpopulation was defined by low engagement. 756 out of 6,050 learners started the survey (12.5\% response rate), and 459 completed it (61\% completion rate). Missing values were multiply imputated using chained equations based on responses to all survey questions\footnote{reference to MICE}. All estimates were pooled across ten imputations, with standard errors adjusted for added variance. 85\% of respondents were male, 35.6 years old on average ($SD=12.5$), and 80\% had achieved a bachelors' or more advanced degree.
...
Two research assistants independently developed a codebook for the resulting 448 non-empty open responses. Their codebooks were consolidated and applied on Mechanical Turk, where 250 randomly selected responses were each coded by four `classification experts'. Specific updates to the initial codebook were informed by code frequency, code correlations, and inter-coder agreement. The updated codebook was then applied by Mechanical Turk `classification experts' to the 198 remaining open responses (four coders per response).

Besides these open responses, learners reported the difficulty of the course material and the extent to which their progress was hindered by a number of obstacles (see Table [XX]). The survey also included a measure of learners' sense of social and academic fit \cite{walton2007question} (17 items, $M=4.66, SD=0.71, \alpha=0.86$); mindset (4 items on the nature of intelligence and talent\footnote{Items were adpated from the mindset questionnaire available at \url{http://mindsetonline.com/testyourmindset/}.}; $M=4.44, SD=0.92, \alpha=0.77$), goal striving (4 items on motivation, perceived importance, committment, and confidence; $M=3.12, SD=0.95, \alpha=0.82$), and one item on volition (about lower-priority distractions hindering the learner's course progress; $M=2.73, SD=1.29$).
...
Reasons for learner attrition were measured using open response as well as multiple-choice questions. As we were interested in learners who left the course unsatisfied, we focused on the subpopulation that reported being only somewhat or less satisfied with their progress in the course (73\% of respondents). Almost half (47\%) the respondents found the course somewhat, very, or extremely difficult. When asked about the extent their progress was hindered by certain challenges, 18\% reported problems with computer or Internet access, 8\% reported technical difficulties, 17\% had difficulties with their level of English, and 24\% found the course requirements unclear. While two in three (66\%) respondents were substantially held back by other committments that took up time they had planned to spend on the course, only one in four (25\%) indicated being hindered by distractions of lower priority than the course (i.e. volition). The two measures, which only differ in the specified level of priority, were not significantly correlated, $r=.05, t_{548}=1.3 p=.21$. 
...
\bottomrule
% \multicolumn{4}{l}{Two-sided $t$-test: $^{***} < .001$, $^{**} < .01$, $^{*} < .05$} \\
\end{tabular}
\end{table*}
